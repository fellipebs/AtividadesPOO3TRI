...
namespace DAL
{
    class DALProjeto : DALMysql
    {
        ...
        public string SelectColumnPorId(string tabela, string condicao, string coluna)
        {
            try
            {
                DataTable dt = new DataTable();
                dt = SelectWhere(tabela, condicao);
    ...
    abstract class DALMysql
    {
        ...
        private void Conectar()
        {
            try
            {
                con = new MySqlConnection($@"server={servidor};uid={usuario};pwd={senha};database={bancodedados};Character Set=utf8");
                con.Open();
            }
